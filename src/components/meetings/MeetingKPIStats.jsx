...
export default function MeetingKPIStats({ meetings }) {
  ...
  // Agenda compliance
  const meetingsWithAgenda = meetings.filter(m => m.has_agenda || (m.agenda_sections && m.agenda_sections.length > 0) || (m.agenda_html && m.agenda_html.replace(/<[^>]*>/g, '').trim().length > 0) || (m.description && m.description.trim().length > 0));
