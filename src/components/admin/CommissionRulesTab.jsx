  ...
  const handleOpenDialog = (rule = null) => {
    if (rule) {
      ...
      setFormData({
        ...
        precon_phase_availability: rule.precon_phase_availability || formData.precon_phase_availability,
        construction_phase_availability: rule.construction_phase_availability || formData.construction_phase_availability,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Commission Rules</h2>
          <p className="text-sm text-slate-500">Define commission tiers and rates</p>
        </div>
      ...
      <div className="grid gap-6">
        {rules.map((rule) => (
          <Card key={rule.id}>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${rule.is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700'}`}>
            ...
            <CardContent>
              {rule.notes && <p className="text-sm text-slate-600 mb-4">{rule.notes}</p>}
                  ...
                  <TableBody>
                    {rule.tiers?.map((tier, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell>{tier.max_volume ? `$${tier.max_volume.toLocaleString()}` : 'Unlimited'}</TableCell>
                        <TableCell className="font-semibold text-amber-600">{tier.commission_rate}%</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
                ...
                <Card key={index} className="p-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      ...
                      <Input value={tier.tier_name} onChange={(e) => handleTierChange(index, 'tier_name', e.target.value)} />
                    </div>
                    <div>
                      <Label>Min Volume ($)</Label>
                      <Input type="number" value={tier.min_volume} onChange={(e) => handleTierChange(index, 'min_volume', parseFloat(e.target.value) || 0)} />
                    </div>
                    <div>
                      <Label>Max Volume ($)</Label>
                      <Input type="number" placeholder="Unlimited" value={tier.max_volume || ''} onChange={(e) => handleTierChange(index, 'max_volume', e.target.value ? parseFloat(e.target.value) : null)} />
                    </div>
                    <div className="col-span-2">
                      <Label>Commission Rate (%)</Label>
                      <div className="flex gap-2">
                        <Input type="number" step="0.1" value={tier.commission_rate} onChange={(e) => handleTierChange(index, 'commission_rate', parseFloat(e.target.value) || 0)} />
                        {formData.tiers.length > 1 && (
                          <Button type="button" variant="outline" onClick={() => handleRemoveTier(index)} className="text-red-600">
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        )}
                      </div>
